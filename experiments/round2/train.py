"""Round2 mechanism ablations. Independent copy; never alters round1 artifacts."""
import os
...
def main():
    ...
    parser.add_argument("--method", choices=["A", "B", "C", "C_state_only", "OT_pull"], required=True)
    ...
    gradient_mode = {"C_state_only": "state_only", "OT_pull": "ot_pull"}.get(args.method, "full")
    backprop_source = args.method in {"C", "C_state_only", "OT_pull"}
    ...
    with torch.random.fork_rng(devices=devices):
        ...
        flow = Flow().to(device) if args.method not in {"A", "OT_pull"} else None
    ...
    config.update(batch_size=32, patch_size=7, train_n=len(train_loader.dataset),
                  ...
                  target_feature_mode="train, no-grad in A/B/C; target updates BN buffers",
                  ...
                  flow_source_gradient=gradient_mode if backprop_source else "detached",
                  experiment="round2_mechanism", parent_protocol_sha256=file_hash(parent_lock),
        ...
        for step in range(1, len(train_loader)):
            ...
            if args.method != "A":
                fm = matching_loss(flow, source_features, target_features, labels, q,
                                   generator, backprop_source=backprop_source,
