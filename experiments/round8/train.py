"""Round8: prototype-weighted OT mass with unchanged reverse Flow."""
import os
...
from flow import ReverseFlow, reverse_matching_loss
...
def main():
    ...
    parser.add_argument("--method", choices=["reverse_flow"], required=True)
    ...
    parser.add_argument("--pairing", choices=["classifier_q", "classifier_q_batch_prototype_mass"],
                        default="classifier_q_batch_prototype_mass")
    ...
    config.update(batch_size=32, patch_size=7, train_n=len(train_loader.dataset),
                  ...
                  feature_dim=288, fm_weight=1., transport_direction="target_to_source",
                  flow_class_condition="none", inference_steps=4,
                  ot_reg=.05, ot_iterations=100,
                  pairs_per_present_class=32,
                  flow_source_gradient="detached; CE-only backbone trajectory",
                  experiment="round8_batch_prototype_mass", pairing_threshold=.95,
                  prototype_temperature=.05,
                  prototype_source="detached current source batch features",
                  prototype_role="OT target mass only; classifier q retains candidate gate",
                  empty_pair_policy="skip Flow optimizer step",
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for step in range(1, len(train_loader)):
            ...
            with torch.no_grad():
                ...
                model(target_flip)
            prototype_q = None
            if args.pairing == "classifier_q_batch_prototype_mass":
                with torch.no_grad():
                    sums = torch.zeros(7, source_features.shape[1], device=device)
                    counts = torch.bincount(labels, minlength=7)
                    sums.index_add_(0, labels, source_features.detach())
                    prototypes = sums / counts.clamp_min(1)[:, None]
                    similarity = (F.normalize(target_features, dim=1)
                                  @ F.normalize(prototypes, dim=1).T)
                    similarity[:, counts == 0] = -torch.inf
                    prototype_q = (similarity/.05).softmax(1)
            fm, pair_n = reverse_matching_loss(flow, source_features, target_features,
                                       labels, q, generator, prototype_q)
            loss = ce + fm
