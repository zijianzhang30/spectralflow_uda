...
@torch.no_grad()
def ot_pairs(source, target, labels, q, generator, prototype_q=None,
             reg=.05, iterations=100):
    costs = (1 - F.normalize(source, dim=1) @ F.normalize(target, dim=1).T).cpu()
    labels, q = labels.cpu(), q.cpu()
    if prototype_q is not None:
        prototype_q = prototype_q.cpu()
    ...
    for c in range(7):
        ...
        b = q[candidates, c].double()
        if prototype_q is not None:
            b = b * prototype_q[candidates, c].double()
        b = b.clamp_min(1e-12)
        b = b/b.sum()
...
def reverse_matching_loss(flow, source, target, labels, q, generator,
                          prototype_q=None):
    si, ti = ot_pairs(source.detach(), target.detach(), labels, q.detach(),
                      generator, None if prototype_q is None else prototype_q.detach())
    if not len(si):
