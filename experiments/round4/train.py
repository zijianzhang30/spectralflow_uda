"""Round4 target-consistency experiment; earlier rounds stay frozen."""
import os
...
def main():
    ...
    parser.add_argument("--method", choices=["A", "target_consistency", "flow_gate_consistency"], required=True)
    ...
    config.update(batch_size=32, patch_size=7, train_n=len(train_loader.dataset),
                  ...
                  feature_dim=288, fm_weight=1., target_ce_max_weight=.5,
                  target_ce_start_epoch=20, target_ce_warmup_epochs=20,
                  pseudo_threshold=.95,
                  ot_reg=.05, ot_iterations=100,
                  pairs_per_present_class=32,
                  flow_source_gradient="detached; target-noise CE updates target encoder",
                  experiment="round4_target_consistency", parent_protocol_sha256=file_hash(parent_lock),
    ...
    for epoch in range(1, args.epochs+1):
        ...
        ce_sum = fm_sum = aux_sum = selected_sum = 0.
        aux_weight = .5*min(max((epoch-20)/20, 0.), 1.)
        for step in range(1, len(train_loader)):
            ...
            # Preserve official six backbone passes and BN update order.
            with torch.no_grad():
                model(source_noise)
            if flow is None:
                with torch.no_grad():
                ...
                _, target_noise_logits = model(target_noise)
                pseudo, selected = pseudo_label_mask(
                    flow, model.classifier, target_features, q, args.method,
                    threshold=.95)
                selected_count = int(selected.sum())
                aux_ce = (F.cross_entropy(target_noise_logits[selected], pseudo[selected])
                          if selected_count else target_noise_logits.sum()*0.)
            with torch.no_grad():
                model(source_flip)
                model(target_flip)
            if flow is None:
                ...
                fm = matching_loss(flow, source_features, target_features,
                                   labels, q, generator, backprop_source=False)
            ...
            row = dict(epoch=epoch, step=step, ce=float(ce.detach()),
                       fm=float(fm.detach()), target_ce=float(aux_ce.detach()),
                       target_ce_weight=aux_weight, pseudo_selected=selected_count,
        ...
        metrics = dict(epoch=epoch, lr=lr, ce=ce_sum/(len(train_loader)-1),
                       ...
                       target_ce=aux_sum/(len(train_loader)-1),
                       target_ce_weight=aux_weight,
                       pseudo_selected_per_batch=selected_sum/(len(train_loader)-1),
