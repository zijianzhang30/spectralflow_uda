...
def transport_objective(flow, classifier, source, target, labels, target_q,
                        generator, variant):
    """Classify source-labelled views while controlling the CE-to-Flow gradient.

    All variants share the same OT pairs, detached FM endpoints, and loss
    weights. Target features and pseudo probabilities never receive gradients.
    """
    if variant not in {"flow_transport_detached", "flow_transport_coupled",
                       "linear_transport"}:
    ...
    if variant == "linear_transport":
        displacement = velocity.detach()
    else:
        displacement = flow(detached_start, t0, classes)
        if variant == "flow_transport_detached":
