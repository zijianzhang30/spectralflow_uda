...
class DSANSS(nn.Module):
    ...
    def forward_with_spectral(self, x, y):
        """Return the normal MLUDA outputs plus pre-MBCA spectral features.

        The extra features are the pooled 192-channel spectral-only branch
        from DCRN_02, before spatial concatenation and CrossAttention.  The
        original ``forward`` path and all downstream MLUDA behavior remain
        unchanged.
        """
        features_x, features_y, spectral_x, spectral_y = self.feature_layers(
            x, y, return_spectral=True
        )
        x1 = F.normalize(self.head1(features_x), dim=1)
        x2 = F.normalize(self.head2(features_x), dim=1)
        fea_x = self.fc1(features_x)
        output_x = self.sigmoid(self.fc2(features_x))
        y1 = F.normalize(self.head1(features_y), dim=1)
        y2 = F.normalize(self.head2(features_y), dim=1)
        fea_y = self.fc1(features_y)
        output_y = self.sigmoid(self.fc2(features_y))
        return (features_x, x1, x2, fea_x, output_x,
                features_y, y1, y2, fea_y, output_y,
                spectral_x, spectral_y)

    def forward_with_spectral_spatial(self, x, y, return_spatial_map=False):
        ...
        """
        result = self.feature_layers(x, y, return_spectral=True, return_spatial=True, return_spatial_map=return_spatial_map)
        if return_spatial_map:
            features_x, features_y, spectral_x, spectral_y, spatial_x, spatial_y, spatial_map_x, spatial_map_y = result
        else:
            features_x, features_y, spectral_x, spectral_y, spatial_x, spatial_y = result
        ...
        base = (features_x, x1, x2, self.fc1(features_x), self.sigmoid(self.fc2(features_x)),
                features_y, y1, y2, self.fc1(features_y), self.sigmoid(self.fc2(features_y)),
                spectral_x, spectral_y, spatial_x, spatial_y)
        return base + (spatial_map_x, spatial_map_y) if return_spatial_map else base
...
class DCRN_02(nn.Module):
    ...
    def forward(self, x, y, return_spectral=False, return_spatial=False, return_spatial_map=False):
        ...
        if return_spectral:
            # x1/y1 are the spectral-only branches immediately before the
            ...
            if return_spatial:
                ...
                if return_spatial_map:
                    return F_y2x, F_x2y, spectral_x, spectral_y, spatial_x, spatial_y, x2, y2
