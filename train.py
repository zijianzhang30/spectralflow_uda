"""One explicit trainer for A=CE, B=detached Flow, C=source-gradient Flow."""
import os
...
def main():
    ...
    parser.add_argument("--protocol", choices=["raw", "official_ilda"], default="raw")
    parser.add_argument("--ilda-cache", type=Path)
    parser.add_argument("--data", type=Path, default=DEFAULT_DATA)
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--seed", type=int, default=1341)
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--lr-horizon", type=int, default=100)
    ...
    train_loader, target_loader, val_loader, split = loaders(source, target, gt, args.seed)
    ...
    config.update(batch_size=32, patch_size=7, train_n=len(train_loader.dataset),
                  ...
                  optimizer_reset_each_epoch=True, target_labels_loaded=False,
                  target_sampling="all scene pixels, no label mask",
                  selection="source_val_best", tie_rule="earliest",
                  target_feature_mode="eval, no-grad; preserve all model state and RNG",
                  source_train_labels_hash=array_hash(split["train_labels"]),
                  source_val_labels_hash=array_hash(split["val_labels"]),
                  feature_dim=288, fm_weight=1., ot_reg=.05, ot_iterations=100,
                  pairs_per_present_class=32,
                  flow_source_gradient="state and displacement" if args.method=="C" else "detached",
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for step in range(1, len(train_loader)):
            ...
            fm = ce.new_zeros(())
            if flow is not None:
                # The target-only branch must not alter backbone buffers or main RNG.
                with evaluation(model):
                    target_features, target_logits = model(target_x)
                    q = target_logits.softmax(1)
                fm = matching_loss(flow, source_features, target_features, labels, q,
                                   generator, backprop_source=args.method=="C")
            loss = ce + fm
        ...
        checkpoint = dict(model=model.state_dict(), flow=flow.state_dict() if flow else None,
                          ...
                          source_loader_rng=train_loader.generator.get_state(),
                          target_loader_rng=target_loader.generator.get_state())
    ...
    atomic_json(out/"training_complete.json", dict(epochs=args.epochs, lr_horizon=args.lr_horizon,
                seconds=time.time()-started, target_labels_loaded=False, formal=args.epochs==100,
                selection="source_val_best", selected_checkpoint="best_source_val.pth"))
